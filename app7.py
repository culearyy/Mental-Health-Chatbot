...
# --- 1. Comprehensive Mental Health Disclaimer ---
st.sidebar.markdown(
    """
    ## Important Notice
...
# --- 2. Define the mental health conversation patterns and responses ---
...
def get_response_type(user_input):
    ...
    if any(re.search(r"^(hi|hello|hey|good\s+morning|good\s+afternoon|good\s+evening)", text)):
        return "greeting"
    
    elif any(re.search(r"how\s+are\s+you", text)):
        return "greeting"
    
    elif any(re.search(r"(depress|sad|down|unhappy|blue|hopeless)", text)):
        return "depression"
    
    elif any(re.search(r"(anxi|worry|stress|nervous|panic|overwhelm)", text)):
        return "anxiety"
    
    elif any(re.search(r"(stress|pressure|overwhelm|too\s+much)", text)):
        return "stress"
    
    elif any(re.search(r"(sleep|insomnia|tired|exhausted|rest)", text)):
        return "sleep"
    
    elif any(re.search(r"(relationship|boyfriend|girlfriend|partner|spouse|husband|wife|friend)", text)):
        return "relationships"
    
    elif any(re.search(r"(self.?care|relax|calm|peace)", text)):
        return "self_care"
    
    elif any(re.search(r"(thank|helped|feeling better|appreciate)", text)):
        return "positive"
    
    else:
        # If no specific pattern matches, use general mental health check
        # or default response based on message length
        if len(text) < 20:
...
# Create a container for the conversation with scrolling
...
if user_input:
    ...
    bot_response = generate_mental_health_response(user_input)
