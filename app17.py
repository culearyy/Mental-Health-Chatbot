...
if "mood_history" not in st.session_state:
    st.session_state.mood_history = {}  # ✅ Initialize mood history
...
import random
import difflib  # Helps find the most similar text match
import re
from datetime import datetime
import nltk
@st.cache_data
...
# ✅ Extract only the "intents" list
intents_data = mental_health_data.get("intents", [])

# 🔍 Debugging: Ensure data is loaded
if not intents_data:
    raise ValueError("Error: 'intents' key is missing or empty in intents.json")





# If mental_health_data is a dictionary, convert it to a list first
print("Loaded dataset sample:", list(mental_health_data.items())[:3])
    ...
    **This AI chatbot is for educational and support purposes only** and is not a substitute for professional mental health advice, diagnosis, or treatment.
    ...
    for idx, (mood_text, mood_value) in enumerate(moods.items()):
        ...
        if mood_cols[idx].button(mood_text, key=f"mood_btn_{mood_value}"):
            ...
            st.session_state.mood_history[today] = mood_value  # ✅ Now it won't crash!    
...
# Context memory (stores recent conversation topics)
...
# Define mood categories for tracking
mood_map = {
    "sadness": "😢 Sad",
    "stress": "😟 Stressed",
...
def find_best_response(user_input):
    """Finds the most relevant response from the dataset."""
    ...
    for intent in intents_data:
        for pattern in intent["patterns"]:
            match_score = len(set(user_input.split()) & set(pattern.lower().split())) / len(set(pattern.lower().split()))
            
            if match_score > best_score:  # ✅ Keep the best matching intent
                best_score = match_score
                best_match = intent["responses"]
                best_tag = intent["tag"]

    if best_match and best_score > 0.3:  # ✅ Ensure a strong enough match
        return random.choice(best_match), best_tag  # ✅ Returns BOTH response & tag
 

    return "I'm here to support you. Can you tell me more about how you're feeling?", "default"  # ✅ Now returns two values
...
def generate_mental_health_response(user_input):
    """Generates chatbot response, updates memory & mood tracking."""
    
    # Find best response & tag
    response, _ = find_best_response(user_input)  # ✅ Ignore detected_tag
    return response  # ✅ Now returns only the response string


    # Store context memory (remember last 3 user messages)
    ...
    # Update mood tracker if relevant
...
def generate_mental_health_response(user_input):
    """Generate a chatbot response based on the dataset."""
    
    response, detected_tag = find_best_response(user_input)  # ✅ Get both response and detected tag
...
# Update Mood Tracker if relevant
    ...
    if detected_tag in mood_map:
       st.session_state["current_mood"] = mood_map[detected_tag]  # ✅ Update mood
  # Get response from dataset
    response_type = "deep_response"  # Label it as a deep response
...
def on_send():
    ...
    if user_msg:
        ...
        simulate_bot_typing()  # 🟢 Call directly
...
def simulate_bot_typing():
    ...
    if user_messages:
        ...
        bot_msg = generate_mental_health_response(last_user_msg)  # ✅ Directly returns a single response



        # 🔄 Add memory context for deeper responses
        if len(st.session_state.context_memory) >= 2:
            bot_msg += f" Earlier you mentioned: \"{st.session_state.context_memory[-2]}\". Would you like to talk more about that?"
        ...
        st.session_state.is_typing = False  # Stop typing indicator
