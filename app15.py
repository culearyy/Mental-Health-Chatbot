...
# ✅ Check if conversation exists BEFORE doing anything else
if "conversation" not in st.session_state:
    print("🛑 Conversation was missing! Initializing now.")
    ...
    print("✅ Conversation already exists in session state.")
st.set_page_config(
...
if "current_mood" not in st.session_state:
    st.session_state["current_mood"] = "Neutral"  # ✅ No emojis
...
download_nltk_data()  # Ensures downloads happen only once
...
with st.sidebar:
    ...
    **This AI chatbot is for educational and support purposes only** and is not a substitute for professional mental health advice, diagnosis, or treatment.
    ...
    if st.session_state.current_mood:
        ...
        tips = {
    "Neutral": "Stay mindful and take breaks!",
    "Feeling Low": "Try writing down your thoughts or talking to a friend.",
    "Anxious": "Take deep breaths and focus on the present moment.",
    "Urgent Support Needed": "Please reach out to a trusted person or helpline for immediate support."
}



        
st.info(tips.get(st.session_state.current_mood, "Take care and stay mindful!"))  # ✅ Prevents KeyError
...
try:
    ...
    print("⚠️ Warning: mental_health_responses.json file not found! Using default responses.")
...
def generate_mental_health_response(user_input):
    """Generate a chatbot response based on the dataset with improved matching."""

    user_input = user_input.lower().strip()

    best_match = None
    for entry in mental_health_data:
        context_text = entry["Context"].lower()
        if re.search(rf"\b{re.escape(user_input)}\b", context_text):  # ✅ Allows partial match
            best_match = entry
            break

    if best_match:
        response = best_match["Response"]
        response_type = best_match.get("Category", "default")  # Ensure response type exists
    else:
    ...
    return response, response_type  # ✅ Returns two values
...
def on_send():
    ...
    if user_msg:
        ...
        st.session_state.is_typing = True  # ✅ Set to True before calling bot
        st.session_state.typing_triggered = False  # ✅ Ensure typing is triggered
        st.session_state["input_text"] = ""  # ✅ Clear input box

        simulate_bot_typing()  # ✅ Now properly calls the bot
        time.sleep(0.5)  # ✅ Ensure UI updates before refresh
        st.session_state.is_typing = False  # ✅ Reset after bot response
...
def simulate_bot_typing():
    """Simulate chatbot typing delay and pass session state to thread manually."""

    # ✅ Copy conversation from session state to pass to the thread
    user_messages = list(st.session_state.get("conversation", []))

    def respond(user_messages_copy):
        time.sleep(1)  # ✅ Short delay

        # ✅ Debugging Output
        print("🔍 Inside Thread - Received Messages Copy:", user_messages_copy)

        last_user_msg = next((msg for msg in reversed(user_messages_copy) if msg["role"] == "User"), None)

        if last_user_msg:
            print(f"✅ Processing response for: {last_user_msg['content']}")  

            bot_msg, response_type = generate_mental_health_response(last_user_msg["content"])  
            timestamp = datetime.now().strftime("%H:%M")

            # ✅ Update conversation directly in session state
        # ✅ Ensure conversation exists before modifying it
# ✅ Ensure conversation exists before modifying it
# ✅ Ensure conversation exists before modifying it
if "conversation" not in st.session_state:
    st.session_state.conversation = []
...
if last_user_msg:
    ...
    print(f"🟢 Bot Response Added: {bot_msg}")  # Debugging log


    # ✅ Update UI
    st.session_state.is_typing = False  
# ✅ Ensure bot_msg is always assigned before using it
if last_user_msg:
    bot_msg, response_type = generate_mental_health_response(last_user_msg["content"])
else:
    bot_msg, response_type = "I'm here to listen. Please tell me more.", "default"

# ✅ Prevent infinite reruns and only rerun when bot response changes
if "last_bot_response" not in st.session_state or st.session_state.last_bot_response != bot_msg:
    st.session_state.last_bot_response = bot_msg
    if "rerun_trigger" in st.session_state and not st.session_state.rerun_trigger:
        st.session_state["rerun_trigger"] = True
        st.rerun()
else:
    print("⏳ Preventing infinite loop: No need to rerun again.")
...
def respond():
    ...
    # ✅ Ensure bot_msg is defined before using it
    if last_user_msg:
        bot_msg, response_type = generate_mental_health_response(last_user_msg["content"])
    else:
        bot_msg, response_type = "I'm here to listen. Tell me more.", "default"  # ✅ Fallback response
...
# ✅ Ensure bot stops "typing"
st.session_state.is_typing = False  

# ✅ Explicitly print to confirm the response was generated
if last_user_msg:
    print(f"Processing response for: {last_user_msg['content']}")  # Debugging output
    bot_msg, response_type = generate_mental_health_response(last_user_msg["content"])
else:
    bot_msg, response_type = "I'm here to listen. Please tell me more.", "default"
...
# ✅ Now it's safe to print bot_msg
print(f"🟢 Bot Response Added: {bot_msg}")
...
# ✅ Prevent infinite reruns by checking last response
if "last_bot_response" not in st.session_state or st.session_state.last_bot_response != bot_msg:
    st.session_state.last_bot_response = bot_msg
    if "rerun_trigger" in st.session_state and not st.session_state.rerun_trigger:
        st.session_state["rerun_trigger"] = True
...
# ✅ Copy user messages before starting the thread
user_messages = list(st.session_state.conversation)  

# ✅ Start response thread safely
...
if "rerun_trigger" in st.session_state and st.session_state["rerun_trigger"]:
    st.session_state["rerun_trigger"] = False  # ✅ Prevents infinite loops
...
with tab_chat:
    ...
    if "typing_triggered" not in st.session_state or not st.session_state.typing_triggered:
        st.session_state.typing_triggered = True  # ✅ Prevents duplicate threads
        simulate_bot_typing()  # ✅ Runs only when needed
