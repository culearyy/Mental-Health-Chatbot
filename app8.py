...
# Add a bit of custom CSS for nicer chat bubbles
st.markdown(
    """
    <style>
    .chat-bubble {
...
with chat_container:
    for speaker, message in st.session_state.conversation:
        if speaker == "User":
            st.markdown(
                f"<div class='chat-bubble user-bubble'><strong>You:</strong> {message}</div>",
                unsafe_allow_html=True
            )
        else:
            st.markdown(
                f"<div class='chat-bubble assistant-bubble'><strong>Assistant:</strong> {message}</div>",
                unsafe_allow_html=True
            )
