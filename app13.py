...
def generate_mental_health_response(user_input):
    ...
    # Search for the best matching context in the dataset
    best_match = None
    for entry in mental_health_data:
        if user_input in entry["Context"].lower():
            best_match = entry
            break
    ...
    if best_match:
        ...
        response = random.choice(["I'm here for you.", "Can you tell me more?", "It's okay to feel this way."])
...
def simulate_bot_typing():
    """Simulate chatbot typing delay without blocking UI updates."""
    
    user_messages = list(st.session_state.conversation) if "conversation" in st.session_state else []

    def respond():
        time.sleep(1)  # Short delay

        last_user_msg = next((msg for msg in reversed(user_messages) if msg["role"] == "User"), None)
        ...
        if last_user_msg:
            ...
            # Update session state safely using Streamlit's session_state
            st.session_state.conversation.append({
                "role": "Assistant",
                "content": bot_msg,
                "time": timestamp,
            ...
            st.session_state.is_typing = False  # Ensure bot stops "typing"
            st.session_state.typing_triggered = False  # Reset typing trigger

    threading.Thread(target=respond, daemon=True).start()  # Run in background
